...
describe('format store with list', () => {
  test('with heterogeneous values', () => {
    const $list = createStore([true, 'b', false, 5, 'foo']);
    const $formatted = format`(${$list})`;
    expect($formatted.getState()).toBe('(true, b, false, 5, foo)');
  });

  test('with homogeneous values', () => {
    const $numbers = createStore([5, 6, 7, 8]);
    const $strings = createStore(['a', 'b', 'c', 'd']);
    const $booleans = createStore([true, true, false, true]);

    const $number = format`(${$numbers})`;
    const $string = format`(${$strings})`;
    const $boolean = format`(${$booleans})`;

    expect($number.getState()).toBe('(5, 6, 7, 8)');
    expect($string.getState()).toBe('(a, b, c, d)');
    expect($boolean.getState()).toBe('(true, true, false, true)');
  });
});

test('allows to pass raw values', () => {
  const $result = format`(${1} ${'foo'} ${false} ${true} ${[1, 2]} ${null})`;
  expect($result.getState()).toBe('(1 foo false true 1, 2 null)');
});
